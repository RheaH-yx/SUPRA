...
all_dets = np.loadtxt(dets_path, delimiter=' ', dtype=float)
mapper = PixelMapper(pixel_arr, map_arr)
mot = Map_SORT(mapper, entry_polys, max_age=900, min_hits=3, iou_threshold=0.3, limit_entry=False)
...
for frame_num in trange(1, frame_total, unit="frame"):
    ...
    dets = [Detection(det[2:6], det[6]) for det in dets if int(det[1]) in [1, 2, 3, 4, 5]]
    ...
    for trk, map_point, map_vel, trk_id in trackers:
        # print(f"Tracked object: trk_id={trk_id}, map_point={map_point}")
        ...
        output.append([frame_num, trk_id, *box.flatten().tolist(), map_point[0], map_point[1]])
...
if SAVE:
    # print("Output list length:", len(output))
    # print("Output list content:")
    # for item in output:
    #     print(item)
